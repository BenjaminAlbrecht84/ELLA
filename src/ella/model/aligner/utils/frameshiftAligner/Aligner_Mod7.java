...
public class Aligner_Mod7 {
	...
	public Object[] run(String s1, String s2, AliMode mode, int xDiagonalDrop, DP_Matrix matrix) {
		...
		DP_Matrix_stream matrix2 = new DP_Matrix_stream();
		matrix2.init(n, m);
	...
	private int[] fillingMatrix(int n, int m, DP_Matrix_stream matrix, AliMode mode) {
		...
		while (k < m + n - 4) {
			...
			matrix.newDiagonal();
			int pos = matrix.getPosition(k - L, L);
			for (int i = 0; i < 3; i++) {
				matrix.addValue(i, U >= L ? L : m + 1);
				matrix.addValue(i, U >= L ? U - L + 3 : 2);
			}
			...
			if (L > U) {
				...
				for (int j = L; j < U + 1; j++)
					matrix.replace(2, pos + j - L + 2, a[j - L] + matchScore(k - j, j, false, mode));

				for (int j = L; j < U + 1; j++) {

					int xMax = matrix.getValue(2, pos + j - L + 2);
					...
					if (xMax > maxScore - MyParameters.X_BESTSCORE_DROP) {
						...
						matrix.setValue(0, pos + j - L + 2, MIN_VALUE);
						matrix.setValue(1, pos + j - L + 2, MIN_VALUE);
						matrix.setValue(2, pos + j - L + 2, MIN_VALUE);
					}
...
;
	...
	private void caseAF4(int k, int L, int U, DP_Matrix_stream matrix, int[] a) {
		if (L > 7 && k - U - 1 > 0) {
			int pos = matrix.getPosition(k - L - 1, L - 4);
			int x0 = matrix.getValue(2, pos + 0), x1 = matrix.getValue(2, pos + 1);
			...
			for (int j = fromIndex; j < toIndex; j++)
				a[j - L] = Math.max(a[j - L], matrix.getValue(2, pos + j - x0 - 4 + 2) - F);
	...
	private void caseAFD(int k, int L, int U, DP_Matrix_stream matrix, int[] a) {
		if (L > 6 && k - U - 1 > 0) {
			int pos = matrix.getPosition(k - L - 1, L - 3);
			int x0 = matrix.getValue(2, pos + 0), x1 = matrix.getValue(2, pos + 1);
			Arrays.fill(a, 0, Math.min(Math.max(L, x0 + 3), U + 1) - L, MIN_VALUE);
			int fromIndex = Math.max(L, x0 + 3), toIndex = Math.min(U + 1, x0 + 3 + x1 - 2);
			for (int j = fromIndex; j < toIndex; j++)
				a[j - L] = matrix.getValue(2, pos + j - x0 - 3 + 2);
	...
	private void caseAF2(int k, int L, int U, DP_Matrix_stream matrix, int[] a) {
		if (L > 5 && k - U - 1 > 0) {
			int pos = matrix.getPosition(k - L - 1, L - 2);
			int x0 = matrix.getValue(2, pos + 0), x1 = matrix.getValue(2, pos + 1);
			Arrays.fill(a, 0, Math.min(Math.max(L, x0 + 2), U + 1) - L, MIN_VALUE);
			int fromIndex = Math.max(L, x0 + 2), toIndex = Math.min(U + 1, x0 + 2 + x1 - 2);
			for (int j = fromIndex; j < toIndex; j++)
				a[j - L] = Math.max(a[j - L], matrix.getValue(2, pos + j - x0 - 2 + 2) - F);
	...
	private void caseX(int k, int L, int U, DP_Matrix_stream matrix) {
		int pos = matrix.getPosition(k - L, L);
		for (int j = L; j < U + 1; j++)
			matrix.addValue(2, matrix.getValue(1, pos + j - L + 2));
		for (int j = L; j < U + 1; j++)
			matrix.replace(2, pos + j - L + 2, matrix.getValue(0, pos + j - L + 2));
	}

	private void caseD(int k, int L, int U, DP_Matrix_stream matrix) {
		if (L > 6 && k - U > 0) {
			int pos = matrix.getPosition(k - L, L - 3);
			int x0 = matrix.getValue(2, pos + 0), x1 = matrix.getValue(2, pos + 1);
			matrix.fill(0, 2, Math.min(Math.max(L, x0 + 3), U + 1) - L + 2, MIN_VALUE);
			caseD1(k, L, U, matrix, pos, x0, x1);
			caseD2(k, L, U, matrix, pos, x0, x1);
			matrix.fill(0, Math.max(Math.min(U + 1, x0 + 3 + x1 - 2), L) - L + 2, U + 1 - L + 2, MIN_VALUE);
		} else
			for (int j = L; j < U + 1; j++)
				matrix.addValue(0, Math.max(matrix.X(k - j, j - 3) - gop, matrix.D(k - j, j - 3) - gep));
	}

	private void caseD1(int k, int L, int U, DP_Matrix_stream matrix, int xPos, int x0, int x1) {
		int fromIndex = Math.max(L, x0 + 3), toIndex = Math.min(U + 1, x0 + 3 + x1 - 2);
		for (int j = fromIndex; j < toIndex; j++)
			matrix.addValue(0, matrix.getValue(2, xPos + j - x0 - 3 + 2) - gop);
	}

	private void caseD2(int k, int L, int U, DP_Matrix_stream matrix, int xPos, int x0, int x1) {
		int pos1 = matrix.getPosition(k - L, L);
		int pos2 = matrix.getPosition(k - L, L - 3);
		int d02 = matrix.getValue(0, pos2 + 0);
		int fromIndex = Math.max(L, x0 + 3), toIndex = Math.min(U + 1, x0 + 3 + x1 - 2);
		for (int j = fromIndex; j < toIndex; j++)
			matrix.replace(0, pos1 + j - L + 2, matrix.getValue(0, pos2 + j - d02 - 3 + 2) - gep);
	}

	private void caseY(int k, int L, int U, DP_Matrix_stream matrix) {
		if (L > 3 && k - U - 1 > 0) {
			int pos = matrix.getPosition(k - L - 1, L);
			int x0 = matrix.getValue(2, pos + 0), x1 = matrix.getValue(2, pos + 1);
			matrix.fill(1, 2, Math.min(Math.max(L, x0), U + 1) - L + 2, MIN_VALUE);
			caseY1(k, L, U, matrix, pos, x0, x1);
			caseY2(k, L, U, matrix, x0, x1);
			matrix.fill(1, Math.max(Math.min(U + 1, x0 + x1 - 2), L) - L + 2, U + 1 - L + 2, MIN_VALUE);
		} else
			for (int j = L; j < U + 1; j++)
				matrix.addValue(1, Math.max(matrix.X(k - j - 1, j) - gop, matrix.Y(k - j - 1, j) - gep));
	}

	private void caseY1(int k, int L, int U, DP_Matrix_stream matrix, int xPos, int x0, int x1) {
		int fromIndex = Math.max(L, x0), toIndex = Math.min(U + 1, x0 + x1 - 2);
		for (int j = fromIndex; j < toIndex; j++)
			matrix.addValue(1, matrix.getValue(2, xPos + j - x0 + 2) - gop);
	}

	private void caseY2(int k, int L, int U, DP_Matrix_stream matrix, int x0, int x1) {
		int pos1 = matrix.getPosition(k - L, L);
		int pos2 = matrix.getPosition(k - L - 1, L);
		int y02 = matrix.getValue(1, pos2 + 0);
		int fromIndex = Math.max(L, x0), toIndex = Math.min(U + 1, x0 + x1 - 2);
		for (int j = fromIndex; j < toIndex; j++)
			matrix.replace(1, pos1 + j - L + 2, matrix.getValue(1, pos2 + j - y02 + 2) - gep);
	}

	private Object[] traceback(DP_Matrix_stream matrix, AliMode mode, int[] startingCell, int n, int m) {
	...
	private int[] tracebackRec(DP_Matrix_stream matrix, int i, int j, StringBuilder[] alignment, AliMode mode) {
