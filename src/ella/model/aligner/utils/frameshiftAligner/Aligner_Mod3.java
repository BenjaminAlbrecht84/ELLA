		...
		while (k < m + n - 4) {
			...
			if (L > U) {
				...
				if (L > 3 && k - U - 1 > 0) {
					...
					for (int j = L; j < Math.min(Math.max(L, X_diagonal[0]), U + 1); j++)
						diagonals[1][j - L + 2] = MIN_VALUE;
					for (int j = Math.max(L, X_diagonal[0]); j < Math.min(U + 1, X_diagonal[0] + X_diagonal[1] - 2); j++)
						diagonals[1][j - L + 2] = X_diagonal[j - X_diagonal[0] + 2] - gop;
					for (int j = Math.max(L, X_diagonal[0]); j < Math.min(U + 1, X_diagonal[0] + X_diagonal[1] - 2); j++)
						diagonals[1][j - L + 2] = Math.max(diagonals[1][j - L + 2], Y_diagonal[j - Y_diagonal[0] + 2] - gep);
					for (int j = Math.max(Math.min(U + 1, X_diagonal[0] + X_diagonal[1] - 2), L); j < U + 1; j++)
						diagonals[1][j - L + 2] = MIN_VALUE;
				} else
					for (int j = L; j < U + 1; j++)
				...
				if (L > 6 && k - U > 0) {
					...
					for (int j = L; j < Math.min(Math.max(L, X_diagonal[0] + 3), U + 1); j++)
						diagonals[0][j - L + 2] = MIN_VALUE;
					for (int j = Math.max(L, X_diagonal[0] + 3); j < Math.min(U + 1, X_diagonal[0] + 3 + X_diagonal[1] - 2); j++)
						diagonals[0][j - L + 2] = X_diagonal[j - X_diagonal[0] - 3 + 2] - gop;
					for (int j = Math.max(L, X_diagonal[0] + 3); j < Math.min(U + 1, X_diagonal[0] + 3 + X_diagonal[1] - 2); j++)
						diagonals[0][j - L + 2] = Math.max(diagonals[0][j - L + 2], D_diagonal[j - D_diagonal[0] - 3 + 2] - gep);
					for (int j = Math.max(Math.min(U + 1, X_diagonal[0] + 3 + X_diagonal[1] - 2), L); j < U + 1; j++)
						diagonals[0][j - L + 2] = MIN_VALUE;
				} else
					for (int j = L; j < U + 1; j++)
				...
				if (L > 6 && k - U - 1 > 0) {
					...
					for (int j = L; j < Math.min(Math.max(L, X_diagonal[0] + 3), U + 1); j++)
						a[j - L] = MIN_VALUE;
					for (int j = Math.max(L, X_diagonal[0] + 3); j < Math.min(U + 1, X_diagonal[0] + 3 + X_diagonal[1] - 2); j++)
						a[j - L] = X_diagonal[j - X_diagonal[0] - 3 + 2];
					for (int j = Math.max(Math.min(U + 1, X_diagonal[0] + 3 + X_diagonal[1] - 2), L); j < U + 1; j++)
						a[j - L] = MIN_VALUE;
				} else
					for (int j = L; j < U + 1; j++)
				...
				if (L > 5 && k - U - 1 > 0) {
					...
					for (int j = L; j < Math.min(Math.max(L, X_diagonal[0] + 2), U + 1); j++)
						a[j - L] = MIN_VALUE;
					for (int j = Math.max(L, X_diagonal[0] + 2); j < Math.min(U + 1, X_diagonal[0] + 2 + X_diagonal[1] - 2); j++)
						a[j - L] = Math.max(a[j - L], X_diagonal[j - X_diagonal[0] - 2 + 2] - F);
					for (int j = Math.max(Math.min(U + 1, X_diagonal[0] + 2 + X_diagonal[1] - 2), L); j < U + 1; j++)
						a[j - L] = MIN_VALUE;
				} else
				...
				if (L > 7 && k - U - 1 > 0) {
					...
					for (int j = L; j < Math.min(Math.max(L, X_diagonal[0] + 4), U + 1); j++)
						a[j - L] = MIN_VALUE;
					for (int j = Math.max(L, X_diagonal[0] + 4); j < Math.min(U + 1, X_diagonal[0] + 4 + X_diagonal[1] - 2); j++)
						a[j - L] = Math.max(a[j - L], X_diagonal[j - X_diagonal[0] - 4 + 2] - F);
					for (int j = Math.max(Math.min(U + 1, X_diagonal[0] + 4 + X_diagonal[1] - 2), L); j < U + 1; j++)
						a[j - L] = MIN_VALUE;
				} else
