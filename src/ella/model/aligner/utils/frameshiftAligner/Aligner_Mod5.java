	...
	private int[] fillingMatrix(int n, int m, DP_Matrix_unsafe matrix, int xDiagonalDrop, AliMode mode) {
		...
		while (k < m + n - 4) {
			...
			for (int j = L; j < U + 1; j++) {
				...
				diagonals[2].set(j - L + 2, Math.max(diagonals[1].get(j - L + 2), diagonals[0].get(j - L + 2)));
				...
				diagonals[2].set(j - L + 2, Math.max(matchMax, diagonals[2].get(j - L + 2)));
