...
public class BlastStatisticsHelper {
    ...
    private static String[] table = new String[]{"Matrix open extension lambda K H",
            ...
            "BLOSUM90  10 1  0.290  0.0750 0.280", "BLOSUM90  9  2  0.310  0.120  0.460", "BLOSUM90  8  2  0.300  0.0990 0.390",
            ...
            "BLOSUM62  11 1  0.267  0.0410 0.140", "BLOSUM62  11 2  0.297  0.0820 0.270", "BLOSUM62  10 2  0.291  0.0750 0.230",
            ...
            "BLOSUM50  13 2  0.193  0.0350 0.120", "BLOSUM50  13 3  0.212  0.0630 0.190", "BLOSUM50  12 3  0.206  0.0550 0.170",
            ...
            "BLOSUM50  12 2  0.181  0.0250 0.0950", "BLOSUM50  19 1  0.212  0.0570 0.180",
            ...
            "BLOSUM45  14 2  0.195  0.0320 0.100", "BLOSUM45  13 3  0.207  0.0490 0.140", "BLOSUM45  12 3  0.199  0.0390 0.110", "BLOSUM45  11 3  0.190  0.0310 0.0950", "BLOSUM45  10 3  0.179  0.0230 0.0750",
            "BLOSUM45  16 2  0.210  0.0510 0.140", "BLOSUM45  15 2  0.203  0.0410 0.120",
            ...
            "PAM250	14	2	0.182	0.024	0.073", "PAM250	15	3	0.205	0.049	0.13", "PAM250	17	2	0.204	0.047	0.12", "PAM250	14	3	0.200	0.043	0.12",
            ...
            "PAM250	19	1	0.192	0.029	0.083", "PAM250	18	1	0.183	0.021	0.070",
            "PAM250	11	3	0.174	0.020	0.070", "PAM250	13	2	0.171	0.017	0.059", "PAM250	17	1	0.171	0.014	0.052",
            "PAM30	9	1	0.294	0.11	0.61", "PAM30	10	1	0.309	0.15	0.88", "PAM30	7	2	0.305	0.15	0.87",
            "PAM30	6	2	0.287	0.11	0.68", "PAM30	5	2	0.264	0.079	0.45",
            "PAM30	8	1	0.270	0.072	0.40",
            "PAM70	10	1	0.291	0.91	0.41", "PAM70	11	1	0.305	0.12	0.52", "PAM70	8	2	0.301	0.12	0.54", "PAM70	7	2	0.286	0.093	0.43",
    ...
    public static HashMap<String, ArrayList<String>> matrix2penalties = new HashMap<>();
    ...
    static {
        for (int i = 1; i < table.length; i++) {
            ...
            if (!matrixList.contains(name))
                matrixList.add(name);
            matrix2penalties.putIfAbsent(name, new ArrayList<>());
            String penalties = split[1] + "/" + split[2];
            matrix2penalties.get(name).add(penalties);
        }
    }

    public static void init(ScoringMatrix matrix, long referenceLetters) {
        n = referenceLetters;
        double[] blastStatistics = getStatistics(matrix);
    ...
    public static double[] getStatistics(ScoringMatrix matrix) {
        for (String line : table) {
            if (line.startsWith(matrix.getType())) {
                String[] tokens = line.split("\\s+");
                if (tokens.length == 6) {
                    int gop = Integer.parseInt(tokens[1]);
                    int gep = Integer.parseInt(tokens[2]);
                    if (gop == matrix.getGapOpen() && gep == matrix.getGapExtend()) {
    ...
    // minimum score needed s.t. the probably of having 1 random seed hit is less than 0.01
    public static int minSeedScore(int m) {
        return (int) Math.round((1. / LAMBDA) * (Math.log(m * n * K) - 1.52718));
    }

    // minimum score needed as calculated by LAST
    public static int minSeedScoreLast(int m) {
        double x = Math.max(1., 1000.0 * n / (MyParameters.MULTIPLICITY * MyParameters.TOTAL_BATCHES));
        double t = 1. / LAMBDA;
        return (int) Math.round(t * (Math.log(x) + 0.5));
    }

    public static int lastMinSeedScore(int m) {
        return (int) Math.round((1. / LAMBDA) * Math.log(1000. * n / m) + 0.5);
    }
    ...
    public static double getEG2Value(int alignmentScore) {
        return K * Math.pow(10, 9) * Math.pow(10, 9) * Math.exp(-LAMBDA * alignmentScore);
    }
