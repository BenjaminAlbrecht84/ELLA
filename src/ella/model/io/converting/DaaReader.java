...
public class DaaReader {
    ...
    private int last_p;
    private SimpleIntegerProperty totalProgress = new SimpleIntegerProperty();
    private long numOfRecords;

    public DaaReader(File daaFile, boolean verbose) {
        this.verbose = verbose;
        this.daaFile = daaFile;
        daaHeader = new DaaHeader(daaFile);
        daaHeader.loadAllReferences();
        if (verbose)
            daaHeader.print();
    }
    ...
    public ArrayList<DaaHit> parseAllHits(int cores) {
        ...
        readId2Hits = new ConcurrentHashMap<>();

        System.out.println("OUTPUT>Parsing " + daaHeader.getNumberOfQueryRecords() + " query records...");

        last_p = 0;
        allParsedRecords = new AtomicLong(0);
        numOfRecords = daaHeader.getNumberOfQueryRecords();

        int chunk = (int) Math.ceil((double) daaHeader.getNumberOfQueryRecords() / (double) cores);
        ArrayList<Thread> allParser = new ArrayList<Thread>();
        for (int l = 0; l < daaHeader.getNumberOfQueryRecords(); l += chunk) {
            int r = (l + chunk) < daaHeader.getNumberOfQueryRecords() ? l + chunk : (int) daaHeader.getNumberOfQueryRecords();
            int[] bounds = {l, r};
        ...
        public void run() {
            ...
            try {
                ...
                try {
                    ...
                    raf.seek(daaHeader.getLocationOfBlockInFile(daaHeader.getAlignmentsBlockIndex()));
                    for (int i = 0; i < daaHeader.getNumberOfQueryRecords(); i++) {
                        ...
                        if (i >= bounds[0] && i < bounds[1]) {
                            ...
                            while (hitBuffer.position() < hitBuffer.capacity()) {
                                ...
                                h.parseHitProperties(daaHeader, hitBuffer, true);
                            ...
                            if (i != 0 && i % 10 == 0) {
                                int p = (int) Math.round(((double) allParsedRecords.addAndGet(10) / (double) numOfRecords) * 100.);
                                reportProgress(p);
                            }
    ...
    public SimpleIntegerProperty totalProgressProperty() {
